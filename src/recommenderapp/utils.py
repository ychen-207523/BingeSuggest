...
def create_or_update_discussion(db, data):
    ...
    """
    cursor = db.cursor()
    cursor.execute("SELECT * from Discussion where imdb_id = %s LIMIT 1",(data['imdb_id'],))
    result = cursor.fetchone()
    if result == None:
        comments = [
            {
        ...
        cursor.execute("Insert INTO Discussion (imdb_id, comments) values(%s,%s)",(data['imdb_id'],json.dumps(comments)))
        db.commit()
    else:
        ...
        cursor.execute("Update Discussion set comments = %s where imdb_id = %s",(json.dumps(comments),data['imdb_id'],))
        db.commit()
    return (jsonify([ {
                "user": data['user'],
                "comment": data['comment']
